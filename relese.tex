...
%\end{enumerate}
\end{condition}
Notice that these are the same conditions than in Lemma \ref{lemma:rfper} but changing $m$ for $m-1$, sence as we mentioned the kernel loses one order of regularity with respect to the parametrization.  Now we have the holomorphic extension of $V_{\br,\bp}$.
\begin{lemma}
\label{lemma:vcomp} 
...
\mathcal{L}(\IU^s, \IY^{s+1}),
\end{align*}
have holomorphic extension in $K^1_{\xi_1} \times K^2_{\xi_2}$. Moreover, for any $(\br,\bp) \in K^1_{\xi_1} \times K^2_{\xi_2}$, the maps $V_{\br,\bp} \in \mathcal{L}(\IT^s, \IW^{s+1})$, $V_{\br,\bp} \in \mathcal{L}(\IT^s, \IW^{s+1})$ are compact. 
\end{enumerate}
\end{lemma}
\begin{proof} 
...
Lemmas \ref{lemma:vcomp}, and \ref{lemma:wcomp} ensure that exist a region such that each component of the weakly-singular and hyper-singular operator have holomorphic extension. The extension of the operator has to be done in the intersection of all the regions, now we describe the condition that characterize the intesection of the regions. 
...
\todo{cambiar conds del lf con teorema 6.1.3, y ver analisis de sf, mirar lo de elasticidad tambien}
