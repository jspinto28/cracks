...
Let $m \in \IN$, $h \in [0,1]$, and consider a bi-periodic function $g \in \cmspaceh{m}{h}{[-\pi,\pi]\times[-\pi,\pi]}{\IC}$, then given $s_1, s_2$, non negative real numbers with integer parts denoted by $[s_1], [s_2]$ and fractionals parts $\{s_1\},\{s_2\}$ respectively, such that at least one of the following conditions are met 
\begin{enumerate}
\item 
...
\end{cases}
\end{align}
and noticing that for $\{s_1\} =\{s_2\} = 0$ the bound can be reduced to $s_1 +s_2 \leq m+h$.
\end{proof}

\begin{remark}
\label{rem:simplerbound}
A simpler but less tight condition is that the result of previous lemma holds whenever, $s_1+s_2 +\frac{1}{2} < m +h $.
\end{remark}
...
This last condition could be relaxed to the better result $s_1+s_2 < m$ if we consider functions of $\cmspace{m}{[-\pi,\pi]\times[-\pi,\pi]}{\IC}$ with derivatives of order $m+1$ integrables (bounded variation), which is comparable with the previous lemma with $h=1$, more restrictive condition but the subspaces of $\cmspace{m}{[-\pi,\pi]\times[-\pi,\pi]}{\IC}$  with $m+1$ integrable derivatives is larger than $\cmspaceh{m}{1}{[-\pi,\pi]\times[-\pi,\pi]}{\IC}$. 
...
\begin{lemma}
\label{lemma:rfper}
Let $m \in \IN$, $h \in [0,1]$ and $s_1, s_2 \in \IR$ such that $s_1 \leq s_1$, thus if one of the following conditions is satisfied: 
\begin{enumerate}
\item $|s_1| \leq \frac{1}{2}$, $|s_2| \leq \frac{1}{2}$ and $\frac{3}{2} < m+h$.
\item 
$|s_1| > \frac{1}{2}$, $|s_2| \leq \frac{1}{2}$, and  $1+|s_1| < m+h$.
%\begin{align*}
%\frac{1}{2}+ |s_1| < m+h, \text{ for } \{|s_1|\} < \frac{1}{2}, \\ 
%1+ |s_1| < m+h, \text{ for } \{|s_1|\} \geq \frac{1}{2}
%\end{align*}
\item 
$|s_1| \leq \frac{1}{2}$, $|s_2|>\frac{1}{2}$, and $1+|s_2| < m+h$.
%\begin{align*}
%\frac{1}{2}+ |s_2| < m+h, \text{ for } \{|s_2|\} < \frac{1}{2}, \\ 
%1+ |s_2| < m+h, \text{ for } \{|s_2|\} \geq \frac{1}{2}
%\end{align*} 
\item 
$|s_1| > \frac{1}{2}$, $|s_2|>\frac{1}{2}$, and $|s_1|+|s_2|+\frac{1}{2} < m+h$.
%\begin{align*}
...
\begin{enumerate}
\item $s< \frac{-3}{2}$, and $-2s-\frac{1}{2} < m+h$
\item 
$\frac{-3}{2} \leq s < \frac{-1}{2}$,and  $ 1-s < m+h$
\item 
$\frac{-1}{2} \leq s \leq \frac{1}{2}$, and $2+s < m+h$
\item 
$s > \frac{1}{2}$, and $2s + \frac{3}{2} < m+h$.
\end{enumerate}
