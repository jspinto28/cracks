...
\todo{comentario de lo que viene }

\begin{lemma}
\label{lemma:Lfoperator}
Let $m \in \IN$, with $m\geq 2$ and $f \in \cmspace{m}{\iinterv}{\IC}$, then $L_f \in \mathcal{L}(T^s,W^{s+1})$, for \todo{not needed=???} $|s+1| +|s| <m$. \\Moreover, the map $f \in \cmspace{m}{\iinterv}{\IC} \mapsto L_f \in \mathcal{L}(T^s,W^{s+1})$ is continuous, and we have 
$$
...
\begin{align*}
\| \widehat{J} L_f u \|_{H^s} = \|L_f u  \|_{W^s},
\end{align*}
The regular part of $\widehat{J} L_f u$, can me mapped back to $[-1,1]$ and we obtain the following operator
$$
...
$$
Since $m>1$ from Lemma \ref{lemma:Rfoperator} the later operator is bounded  from $T^s$ to $W^{s+1}$ for every $s \in \IR$. For the singular part, we first notice that this correspond to a traditional weakly-singular operator (order $-1$), and we  can use Theorem \todo{saranen theorem 6.1.1}, but the function $f$ is not arbitrary smooth, hence we need that for a $\nu >\frac{1}{2}$,
\begin{align}
\label{eq:condS}
...
\end{align}
 hence by Lemma \ref{lemma:cmdecay}  we have that an equivalent condition to \eqref{eq:condS} is 
\begin{align*}
2 \max(|s+1|, \nu ) - 2(m+1) \alpha < -1 \\ 
2 \max(|s|, \nu ) - 2(m+1) \beta < -1 
\end{align*}
