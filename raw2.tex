...
\end{align*}
On the other hand the case $m=1$ correspond to the case where the $m$-derivatives are Lipchitz continuous, \todo{thus the derivatives of order $m+1$ exist and are bounded almost everywhere}. Notice that for $m_1, m_2 \in \IN$ and $h_1, h_2 \in [0,1]$, such that $m_1 + h_1 \leq m_2 + h_2$, one has that $\cmspaceh{m_2}{h_2}{A}{B} \subset \cmspaceh{m_1}{h_1}{A}{B}$.
...
\end{align*}
with unspecified constants independent of $f$. If $s_1, s_2$ are integer we get the slightly tighter bound $s_1+s_2 \leq m+h$.
\end{lemma}
\begin{proof}
...
\end{split}
\end{align}
where $[s_1], [s_2]$ denotes the integer parts of $s_1,s_2$ respectively, and $\{s_1\}, \{s_2\}$ are the coresponding fractional parts. From the last inequality is immediate that for the case $\{s_1\}= \{s_2\} =0$ we get 
$$
...
\end{split}
\end{align*}
thus the right hand side is finite only if $h > \gamma_1 + \gamma_2$. However this condition could not used when ever $\gamma_1 +\gamma_2 \geq 1$. For this last case we need to use the result \eqref{eq:smnormeq} of the Appendix \ref{appendix:fracbivariate}. In particular if we assume that $\gamma_1 = \max\{\gamma_1,\gamma_2\}$ we have 
$$
...
\begin{remark}
\todo{remark that the bound is better to only use the bound for the coefficients...}
\end{remark}
