...
\end{align*}
An important property that will play a mayor role in the analysis of the hyper-singular operator are the mapping properties of the derivative, in particular from \todo{ref nicase? o martin} we have \todo{are invertible maps?}
\begin{align*}
...
\end{align}
By the assumption of the invertibility of $F_1(d_{\br}(t,t)^2)= F_1(0)$  the first part is the clasical logarithmic operator which is an invertible operator in $T^s$ \todo{ref nedelec}\footnote{In fact the logarithmic operator is diagonal with eigenvectors the first-kind Chebishev polynomials.} \todo{check invertibility in $U^s$, is an infinite matrix... using the periodic operators is hard to argue ...}. On the other hand the difference in the second part can be expanded by Taylor and we get 
$$
