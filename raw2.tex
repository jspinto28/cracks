...
Throughout the rest of this section, we consider $ m \in \IN$, with $m >1$, and $M$ compact disjoint sets $K^1,\hdots,K^M \subset \rgeo{m}$ and the corresponding positive real numbers $\delta_1, ..,\delta_M$ chosen according to Lemmas \ref{lemma:dwelldef}, and \ref{lemma:dcross}, such that for any pair $j,k \in 1,\hdots,M$ (with $j\neq k$) the conclusion of both lemmas holds for the set $K^j_{\delta_j}$ and $ K^j_{\delta_j}\times K^k_{\delta_k}$, respectively \footnote{this selection of values for the $\delta$ variables is possible by taking the minimum value, such that both lemmas holds for all the combination of $j,k$. }. 
...
\begin{enumerate}
\item 
For $j=1,\hdots,M$, and $s\in \IR$ such that $|s+1|+|s|<m$, let $\xi_j< \delta_j$, then the map 
...
\end{lemma}
\begin{proof} 
We are going to focus in the case of scalar operator $\cP$, as the vector case follows verbatim. We recall the assumptions on the structure of the fundamental solution \eqref{eq:funsolgen}, thus the self interaction can be expressed as 
\begin{align*}
...
If we combine the two parts we have two conclusions. First the map $\br \in K^j_{\delta_j} \mapsto V_{\br,\br} \in \mathcal{L}(T^s,W^{s+1})$ is uniformly bounded. Secondly, using the last point and also the properties of the kernel functions $G^R$ and $G^S$, we can now use Theorem \ref{thrm:abstractholm} and obtain the holomorphic extension of the self interaction operator. 

We first consider the operator whose kernel is given by the singular part. In this case by assumptions on the smoothness of $F_1$ and Lemma \ref{lemma:selfkernell} we have the kernel has an holomorphic, and uniformly bounded extension over $K^j_{\delta_j}$ for every $j$ that is a $\cmspace{m}{\iinterv}{\IC}$ function, and from the latter using Lemma \ref{lemma:Lfoperator}
we see that the associated integral operator has a uniformly bounded extension that is in $\mathcal{L}(\IT^s,\IW^{s+1})$\footnote{extension in the parametrization variable $\br$ and the norm of the operator is bounded by the norm of $F_1$.}. 
