...
Typically $\widetilde{G}$ includes explicit operations on the normal vector, thus the corresponding kernel lost one order of regularity. However, this fact does not any impact as would still give us enough regularity to show the properties of the map in $\mathcal{L}(\mathbb{U}^s, \mathbb{M}^{s-1})$, instead of $\mathcal{L}(\mathbb{U}^s, \mathbb{M}^{s+1})$.
\end{remark}
...
$$
Notice that this function is has almost the same structure that $G(\br_i(t),\br_j(s))$ except for the fact that it lost one degree of smoothness because of the factors $\br_i'$, and $\br_j'$, however, as it was noticed in Remark \ref{remark:gtildereg} this do not have any impact, and we obtain the corresponding results for the Neumann problem.
...
We conclude that this function has holomorphic extension, but it also lose one order of regularity (since the components are in $\cmspaceh{m-1}{h}{\iinterv}{\IC}$). This implies that the proof of Lemma \ref{lemma:vcomp} the corresponding functions $F_1(d_{\br}(t,s)^2)$, $F_2(d_{\br}(t,s)^2)$ are in $\cmspaceh{m-1}{h}{\iinterv}{\IC}$ instead of $\cmspaceh{m}{h}{\iinterv}{\IC}$.  However this do not moddify the condition on $s,m,h$ since this is limited for the factor $\log Q_{\br}(t,s) F_1(d_{\br}(t,s)^2) \in \cmspaceh{m-1}{h}{\iinterv}{\IC}$. Hence, the results for the Dirichlet problems follows. 

For the Neumann problem we use the following Maue's representation formula
