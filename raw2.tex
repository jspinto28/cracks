...
Before we proceeded any further we need to specify how the smoothness of the kernel function $f$ impact the maping propieties of the operators $R_f$, $L_f$, and $S_f$. The following result give a first insight regarding this relation. 
...
Let $m \in \IN$, $h \in (0,1)$, and consider $f \in \cmspaceh{m}{h}{\iinterv}{\IC}$, then given $s_1, s_2$, non negative real numbers, such that  $s_1+s_2 < m+h$, then we have  
\begin{align*}
...
\end{align*}
with unspecified constants independent of $f$. If $h = 0 $, or $h = 1$ the same result hold but with the slightly tighter bound $s_1+s_2 \leq m$.
\end{lemma}
\begin{proof}
...
\label{eq:intdevs}
\begin{split}
\|g\|_{s_1,s_2}^2 \lesssim \sum_{n = -\infty}^\infty \sum_{l = -\infty}^\infty (1+n)^{h_1} (1+l^2)^{h_2} \left\vert\left(\widetilde{\partial_t^{[s_1]} \partial_s^{[s_2]}g(t,s)}\right)_{n,l}\right\vert^2+ \\
 \sum_{n = -\infty}^\infty \sum_{l = -\infty}^\infty (1+n)^{h_1} (1+l^2)^{h_2} \left\vert\left(\widetilde{g}\right)_{n,l}\right\vert^2
,
\end{split}
\end{align}
where $[s_1], [s_2]$ denotes the integer parts of $s_1,s_2$ respectively, and $h_1, h_2$ are the coresponding fractional parts. From the last inequality is immediate that for the case $h_1= h_2 =0$ we get 
$$
...
\begin{align*}
S = \sum_{n = -\infty}^\infty \sum_{l = -\infty}^\infty (1+n)^{h_1} (1+l^2)^{h_2} \left\vert\widetilde{\varrho}_{n,l}\right\vert^2 \lesssim \| \varrho \|^2_{\mathbf{0},(h_1,h_2)}. 
\end{align*}
...
\begin{align*}
\|g\|^2_{s_1,s_2} \lesssim \|g\|^2_{([s_1],[s_2]),(h_1,h_2)}. 
\end{align*}
Now to finish the proof for periodic functions we need to bound the general norm $\|g\|_{\mathbf{m},\mathbf{\gamma}}^2$, in terms of the holder norms \todo{seguir esto..}




Notice that $\|g\|_{m,0} \lesssim \|g\|_{\cmspaceh{m}{0}{[-\pi,\pi]\times [-\pi,\pi]}{\IC}}$, and also since Lipchitz continuous implies bounded weak derivative, $\|g\|_{m+1,0} \lesssim \|g\|_{\cmspaceh{m}{1}{[-\pi,\pi]\times [-\pi,\pi]}{\IC}}$.

On the other hand, for $h \in (0,1)$ we have that for every $\gamma \in (0,1)$ such that $\gamma < h$, 
$\|g\|_{m,\gamma} \lesssim \|g\|_{\cmspaceh{m}{h}{[-\pi,\pi]\times [-\pi,\pi]}{\IC}}$.

Now we consider the sobolev type norm of $g$ given by 
$$
\sum_{n=-\infty}^{\infty}\sum_{l=-\infty}^{\infty}
(1+n^2)^{s_1} (1+l^2)^{s_2} |\widetilde{g}_{n,l}|^2
$$
is bounded by $\|g\|^2_{m,\gamma}$ for every $s_1, s_2$ non negative numbers, such that $s_1 + s_2 \leq m + \gamma$.


By the uniform convergence of the Fourier series of $g$ we have that 
...
\begin{align*}
\|g\|_{m,0}^2   = \sum_{n=-\infty}^\infty \sum_{l=-\infty}^\infty \sum_{|\mathbf{\alpha}| \leq m } n^{2 \alpha_1} l^{2 \alpha_2}|\widetilde{g}_{n,l}|^2,
\end{align*}
we also notice that $(1+n^2)^{[s_1]}(1+l^2)^{[s_2]} \leq \sum_{|\mathbf{\alpha}| \leq m } n^{2 \alpha_1} l^{2 \alpha_2}$ for every positive $s_1,s_2$ such that $[s_1]+ [s_2] \leq m$. Thus we left to prove the bound for fractional orders. Let us consider $\gamma \in (0,1)$ then following \todo{kress we have }
